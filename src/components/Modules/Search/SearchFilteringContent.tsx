...
import BaseSearchBar from '@/atomics/input/BaseSearchBar';
import SelectBox from '@/atomics/select/SelectBox';
        ...
        {activeFilters.length > 0 && (
          <div id="selected-filters" className="flex flex-wrap gap-[8px] mb-4 px-[16px] md:px-[0px]">
            {activeFilters.map((filter) => (
              <div 
                key={filter.key} 
                className="flex items-center bg-[#F0F5FF] text-[#0F72F3] py-1 px-3 rounded-full text-[12px] border border-[#2867DC33]"
              >
            ...
            {activeFilters.length && (
              <button 
                ...
                className="text-[#344054] text-[12px] font-medium cursor-pointer"
              >
                Clear all
              </button>
            )}
